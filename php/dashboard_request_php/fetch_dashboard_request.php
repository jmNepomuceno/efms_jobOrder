...
try {
    ...
    foreach ($summaryData as $row) {
        ...
        switch ($status) {
            ...
            case 'Completed':
            case 'Evaluation': $totals['completed'] += $count; break;
            case 'Pending': $totals['pending'] += $count; break;
            case 'On-Process': $totals['onProcess'] += $count; break;
            case 'Unattended': $totals['unattended'] += $count; break;
            case 'RTR': $totals['rtr'] += $count; break;
        }
    }

    $totalPercentage = $totals['totalRequests'] > 0
        ? round(($totals['completed'] / max(1, ($totals['totalRequests'] - $totals['correction']))) * 100, 2)
        : 0;
